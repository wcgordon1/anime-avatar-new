...
const srcContentFolder = join("src", "data");
...
async function configI18n() {
	...
	const defaultTextTranslationObject = textTranslationsObjectValue.match(
		defaultTextTranslationObjectRegex,
	);
	if (!defaultTextTranslationObject) {
		throw new Error(
			`Could not find defaultLocale object in textTranslations object in translationData.json.ts`,
		);
	}
	...
	const defaultRouteTranslationObject = routeTranslationsObjectValue.match(
		defaultRouteTranslationsObjectRegex,
	);
	if (!defaultRouteTranslationObject) {
		throw new Error(
			`Could not find defaultLocale object in routeTranslations object in translationData.json.ts`,
		);
	}
	...
	localesToAdd.forEach((locale) => {
		...
		const localeTextTranslationObject = defaultTextTranslationObjectValue.replace(
			new RegExp(`${defaultLocale}: {`, "g"),
			`${locale}: {`,
		);
		// insert it
		...
		const localeRouteTranslationObject = defaultRouteTranslationObjectValue.replace(
			new RegExp(`${defaultLocale}: {`, "g"),
			`${locale}: {`,
		);
		// insert it
